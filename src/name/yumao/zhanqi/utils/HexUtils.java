...
//          return new String(stringBuffer.toString().getBytes());
    ...
    public static String setStringHeader(String hexStr){
    	String length = Integer.toHexString(hexStr.length()/2-4);
    	if(length.length()==4) length = length.substring(2,4) +  length.substring(0,2);
    	if(length.length()==3) length = length.substring(1,3) + "0" + length.substring(0,1);
    	if(length.length()==2) length = length + "00";
    	if(length.length()==1) length = "0" + length + "00";
    	return "bbcc"+"00000000"+length+hexStr;
    }
    public static int getHexStringLength(String hexStr){
    	...
    	String headerStr = hexStr.substring(12,16);
